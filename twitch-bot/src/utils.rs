...
pub async fn save_token_to_db(db: &DbConn, token: &UserToken) -> Result<(), Error> {
        ...
        twitch_broadcaster::get_by_broadcaster_id(db, token.user_id.as_str()).await?
    {
        ...
        twitch_broadcaster::update(db, model).await?;
    } else {
        // Create a database model for the token
        let token = entity::twitch_broadcaster::Model {
            id: 0,
        ...
        twitch_broadcaster::create(db, token).await?;
    }
    Ok(())
}
...
pub async fn load_token_from_db(
    ...
    if let Some(token_model) =
        twitch_broadcaster::get_by_broadcaster_login(db, broadcaster_login).await?
    {
...
async fn create_user_token_from_model(
    client: &TwitchClient,
    token_model: &entity::twitch_broadcaster::Model,
